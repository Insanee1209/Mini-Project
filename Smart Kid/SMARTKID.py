...
class MainMenu:
    ...
    def setup_ui(self):
        ...
        quit_button = tk.Button(self.root, text="Quit", font=("ROG Fonts", 22), bg='black', fg='white' , width=10, command=self.quit_game)
        quit_button.place(relx=0.5, rely=0.30, anchor='center') 
        self.setup_sound_button()
        
        self.total_score_label = tk.Label(self.root, text=f"Total Score : {total_score}",font=("Comic Sans MS", 20), bg='pink', fg='black' , width=15)
        self.total_score_label.place(relx=0.5, rely=0.42, anchor='center')
        self.top_level_label = tk.Label(self.root, text=f"Top Level : {self.top_level}", font=("Comic Sans MS", 20), bg='lightblue', fg='black' , width=15)
...
class MathQuizGame:
    ...
    def check_answer(self, idx):
        ...
        if selected_answer == self.current_answer:
            ...
            #you did it
            # สร้างหน้าต่างใหม่สำหรับแสดงผลการตอบผิด
            wrong_window = tk.Toplevel(self.root)
            wrong_window.title("แลกคะแนน")
            wrong_window.geometry("520x300")
            wrong_window.resizable(False, False)
            
            # สร้าง Canvas สำหรับวาดพื้นหลังและปุ่ม
            canvas = tk.Canvas(wrong_window, width=520, height=300, bg="brown")
            canvas.pack()
            
            # วาดกรอบสีขาว
            canvas.create_rectangle(10, 12, 510, 280, outline="yellow", width=5)
            
            
            if total_score >= 500:
                canvas.create_text(260, 150, text="คุณต้องการแลก 500 คะแนนจากคะแนนรวม เพื่อเล่นต่อหรือไม่?", fill="white", font=("Arial", 16))
                yes_button = tk.Button(wrong_window, text="Yes", command=lambda: self.use_points_to_continue(wrong_window))
                yes_button.place(x=100, y=220, width=80, height=40)
                no_button = tk.Button(wrong_window, text="No", command=lambda: self.end_game_from_window(wrong_window))
                no_button.place(x=320, y=220, width=80, height=40)
            else:
                self.end_game()

    def use_points_to_continue(self, wrong_window):
        global total_score
        total_score -= 500
        save_total_score(total_score)
    ...
    def end_game_from_window(self, wrong_window):
        wrong_window.destroy()
        self.end_game()

    def return_to_main_menu(self, wrong_window):
        wrong_window.destroy()
        self.end_game()
    ...
    def level_up(self):
        ...
        if self.level < 11:
            self.level += 1
            self.time_left = 90  # Reset time to 60 seconds
            if sound_on:
                level_up_sound.play()
    ...
    def show_win_window(self):
        ...
        if self.level == 10:
            ...
            canvas.create_text(200, 60, text="ยินดีด้วยคุณได้ไปเลเวลถัดไป", fill="white", font=("Arial", 20, "bold"))
        canvas.create_text(200, 100, text=f"Score: {self.score}", fill="white", font=("Arial", 16))
        canvas.create_text(200, 140, text=f"Total Score: {total_score}", fill="white", font=("Arial", 16))
...
class EndScreen:
     ...
     def setup_ui(self):
        ...
        level_label = tk.Label(self.root, text=f"Your Level : {self.level}",  font=("Comic Sans MS", 20), bg='orange', fg='black' , width=13)
        level_label.place(relx=0.5, rely=0.25, anchor='center')

        score_label = tk.Label(self.root, text=f"Your Score: {self.score}",  font=("Comic Sans MS", 20), bg='yellow', fg='black' , width=13)
...
class MathQuizGenerator:
    ...
    def generate_basic_question(self):
        ...
        num1 = random.randint(-10, 99 * self.level)
        num2 = random.randint(100, 990 * self.level)
